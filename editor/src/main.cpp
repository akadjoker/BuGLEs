...
#ifndef BUGL_EDITOR_EMBEDDED_VM
#define BUGL_EDITOR_EMBEDDED_VM 0
#endif
...
int main(int argc, char** argv)
{
    ...
    // Migrate legacy default (Vendor: Droid Sans) to built-in embedded font.
    if (settings.font_choice == EditorFontChoice::DroidSans)
    {
    ...
    auto run_script = [&]()
    {
        ...
        const std::string command =
            QuoteCommandArgument(bugl_path) + " " +
...
#else
        status = "Embedded VM is disabled; stop external runs from terminal/process manager.";
#endif
    };
