...
namespace RaylibBindings
{
    ...
    static int native_SetTextLineSpacing(Interpreter *vm, int argc, Value *args)
    {
        if (argc != 1) return 0;
        SetTextLineSpacing(args[0].asNumber());
        return 0;
    }

    static int native_TextFormat(Interpreter *vm, int argc, Value *args)
    {
        if (argc < 1 || !args[0].isString()) return 0;
        // Simple: just return the format string as-is (no varargs in script)
        vm->pushString(args[0].asStringChars());
        return 1;
    }
    ...
    void register_text(Interpreter &vm)
    {
        ...
        vm.registerNative("SetTextLineSpacing", native_SetTextLineSpacing, 1);

        // Font loading
        vm.registerNative("LoadFont", native_LoadFont, 1);
        vm.registerNative("LoadFontEx", native_LoadFontEx, 2);
        vm.registerNative("UnloadFont", native_UnloadFont, 1);
        vm.registerNative("IsFontReady", native_IsFontReady, 1);
        vm.registerNative("GetFontDefault", native_GetFontDefault, 0);

        // Font drawing
        vm.registerNative("DrawTextEx", native_DrawTextEx, 7);
        vm.registerNative("DrawTextPro", native_DrawTextPro, 10);
        vm.registerNative("MeasureTextEx", native_MeasureTextEx, 4);
    }
