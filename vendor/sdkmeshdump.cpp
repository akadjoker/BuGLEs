    ...
    bool DumpMesh(const wchar_t* fname, const uint8_t* meshData, size_t dataSize, bool fulldump)
    {
        ...
        for (UINT meshIndex = 0; meshIndex < header->NumMeshes; ++meshIndex)
        {
            ...
            else
            {
                ...
                wprintf(L"\t\tBoundsMin: (%f,%f,%f)\n",
                    double(mh.BoundingBoxCenter.x) - double(mh.BoundingBoxExtents.x),
                    double(mh.BoundingBoxCenter.y) - double(mh.BoundingBoxExtents.y),
                    double(mh.BoundingBoxCenter.z) - double(mh.BoundingBoxExtents.z));
                ...
                for (UINT j = 0; j < mh.NumSubsets; ++j)
                {
                    ...
                    if (sIndex == INVALID_SUBSET || sIndex >= header->NumTotalSubsets)
                    {
                    ...
                    else
                    {
                        ...
                        if (primType)
                        {
                            if (subset.MaterialID == INVALID_MATERIAL || subset.MaterialID >= header->NumMaterials)
                            {
        ...
        else
        {
            for (UINT matIndex = 0; matIndex < header->NumMaterials; ++matIndex)
            {
                ...
                wprintf(L"\t\t A: (%f, %f, %f, %f)\n\t\t D: (%f, %f, %f, %f)\n\t\t S: (%f, %f, %f, %f) ^%f\n\t\t E: (%f, %f, %f, %f)\n",
                    double(mat.Ambient.x), double(mat.Ambient.y), double(mat.Ambient.z), double(mat.Ambient.w),
                    double(mat.Diffuse.x), double(mat.Diffuse.y), double(mat.Diffuse.z), double(mat.Diffuse.w),
                    double(mat.Specular.x), double(mat.Specular.y), double(mat.Specular.z), double(mat.Specular.w),
                    double(mat.Power),
                    double(mat.Emissive.x), double(mat.Emissive.y), double(mat.Emissive.z), double(mat.Emissive.w));
            }
        }
